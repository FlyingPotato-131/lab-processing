...
dRe = 0.4021e5 * 0.5 / np.sqrt(data[1, 0:])
...
ax.plot(Re[np.argmax(Urelation)], np.max(Urelation), 'ro')
ax.errorbar(Re, Urelation, dUrelation, dRe, '.')
ax.plot(Re, Urelation, '--')
...
Url1 = np.sqrt(data110[1, 0:] / data[2, 3])
dUrl1 = 0.5 / np.sqrt(data110[1, 0:] * data[2, 3])
Url2 = np.sqrt(data220[1, 0:] / data[2, 10])
dUrl2 = 0.5 / np.sqrt(data220[1, 0:] * data[2, 3])
