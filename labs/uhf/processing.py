...
graphs.plotlsqm(np.cos(np.radians(polData[:, 0]))**2, polData[:, 1], np.ones(np.shape(polData)[0]) * 2 * np.abs(np.cos(np.radians(polData[:, 0])) * np.sin(np.radians(polData[:, 0]))) * np.radians(1), np.ones(np.shape(polData)[0]), title = "intensity over polarization angle", xlabel = "cos^2(α)", ylabel = "receiver voltage V, uV")
