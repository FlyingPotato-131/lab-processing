...
Fs = np.linspace(1, 5, N) #cross section at shockwave in nozzle (parameter)
...
def g_2(pp): #=u2/a1, equation 8.3
	return (pp - 1) / np.sqrt((7/5 * (7/5 - 1) / 2) * (1 + (7/5 + 1) / (7/5 - 1) * pp))

def p21_eq(pp, h): #define equation for finding p2/p1, g(p2 / p1) = h(Fs)
	return g(pp) - h

def p21_eq2(pp, h): #define equation for finding p2/p1, g(p2 / p1) = h(Fs)
	return g_2(pp) - h

#calculate mach numbers
M6 = sci.fsolve(M_eq, 2 * np.ones(N), args=(M5, F2, Fs)) #solve q function equation

M7 = np.sqrt((2 + (gamma - 1) * M6**2) / (2 * gamma * M6**2 - (gamma - 1))) #shockwave mach number relation
...
hFs = M3 * a41 * a37 * a76 * a65 * a54 #h(Fs) = u2 / a1, g(p2/p1) = h(Fs)
...
p21 = sci.fsolve(p21_eq2, np.ones(N), args=(hFs)) #p2/p1
p41 = p21 / fFs #p4/p1
...
p41_exp = (data[:, 1] / 250 * 6 * 9.81e4 + 759*133.3) / (data[:, 0]*133.3)
